...
our $FILENAME_RE = do {
    my $user = qr/[a-z][a-z0-9]{0,9}/i;
    qr/$user(?:-$user|(?:-$user){3})?(?:-[1-9]\d*)?\.sgf/;
};
...
sub is_game_path {
    my ( $self, $path ) = @_;
    my ( $year, $month, $day, $filename ) = ( split m{/}, $path, 5 )[1..4];
...
sub call {
    ...
    my $fetch = URI::Fetch->fetch(
        'http://files.gokgs.com/games' . $request->path,
